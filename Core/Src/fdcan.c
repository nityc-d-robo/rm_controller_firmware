...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
  ...
  if (RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE)
  {
    ...
    {
      memcpy(main1_rx_data, RxData, 8);
      main1_rx_id = RxHeader.Identifier;
      main1_rx_flag = true;
    }
    else if (hfdcan->Instance == FDCAN2)
    {
      memcpy(main2_rx_data, RxData, 8);
      main2_rx_id = RxHeader.Identifier;
      main2_rx_flag = true;
    }
  }
}
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs)
{
  ...
  if (RxFifo1ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE)
  {
    ...
    {
      // 全体命令
      memcpy(motor1_rx_data, RxData, 8);
      motor1_rx_id = RxHeader.Identifier & 0x00F;
      motor1_rx_flag = true;
    }
    else
    {
      memcpy(motor2_rx_data, RxData, 8);
      motor2_rx_id = RxHeader.Identifier & 0x00F;
      motor2_rx_flag = true;
    }
  }
}
