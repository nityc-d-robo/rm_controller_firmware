...
void speed_pid_task(void)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    if (motorstate[i].can_move > 0)
    {
      motorstate[i].can_move -= 1;
    }
    else
    {
    ...
    {
      if (motorstate[i].r_speed_cnt < 1)
      {
        motorstate[i].speed.target_rpm = 0;
      }
      else
      {
        motorstate[i].r_speed_cnt -= 1;
      }
    }
    if (motorstate[i].motor_type == 1)
    {
      ...
      if (motorstate[i].mode == ANGLE && motorstate[i].can_move > 1)
      {
      ...
      else if (motorstate[i].mode == SPEED && motorstate[i].can_move > 1)
      {
        // if (return_count >= 100) {
        if (return_rpms == true)
        {
      ...
      else if (motorstate[i].mode == CURRENT && motorstate[i].can_move > 1)
      {
    ...
    {
      motorstate[i].can_move = 2;
      if (motorstate[i].mode == SPEED && motorstate[i].can_move > 1)
      {
        // ロボストライドの処理
        FDCAN_TxHeaderTypeDef TxHeader;
        uint8_t tx_datas[64] = {0};
      ...
      else if (motorstate[i].mode == ANGLE && motorstate[i].can_move > 1)
      {
        // ロボストライドの処理
        FDCAN_TxHeaderTypeDef TxHeader;
        uint8_t tx_datas[64] = {0};
